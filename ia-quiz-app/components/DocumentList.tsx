...
export default function DocumentList({ supabase, documents, isLoading, onDeleteSuccess }: DocumentListProps) {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-4 text-brand-purple-dark">Documents Analysés</h2>

      {deleteError && ( <p className="p-3 mb-4 bg-red-100 text-red-700 rounded-md text-sm">{deleteError}</p> )}
      {isLoading && <p className="text-gray-600 text-sm">Chargement des documents...</p>}
      
      {!isLoading && documents.length === 0 && (
        <p className="text-gray-500 text-sm text-center py-4">Aucun document téléversé.</p>
      )}
            ...
            return (
              ...
              >
                <div className="flex flex-col flex-grow truncate mr-2">
                  <span className="text-sm font-medium text-brand-purple-dark truncate" title={doc.name}>
                    {doc.name}
                  </span>
                  <span 
                ...
                >
                  ...
                  <span className="ml-1 hidden md:inline text-sm">Supprimer</span>
                </button>
              </li>
            );
          })}
