...
async function processBatch(payload: WebhookPayload) {
  ...
  const updateDocumentStatus = async (status: 'completed' | 'failed', message?: string) => {
  ...
  try {
    ...
    const { data: docData, error: fetchDocError } = await supabaseClient
        ...
        .eq('id', documentId)
        .single();
    if (fetchDocError || !docData) throw new Error(`Impossible de trouver le document ID: ${documentId}`);
    const { user_id, file_path } = docData;

    // 2. Télécharge le fichier JSON contenant TOUS les chunks
    const { data: chunkFileData, error: downloadError } = await supabaseClient.storage
      .from(CHUNK_STORAGE_BUCKET)
      .download(chunkFilePath);
    if (downloadError) throw new Error(`Erreur téléchargement chunks JSON: ${downloadError.message}`);
    ...
    if (batchTexts.length === 0) {
      console.log(`[+] (Batch) Doc ${documentId}: Terminé. Tous les lots sont traités.`);
      await updateDocumentStatus('completed');
      
      // (Optionnel) Supprime le fichier JSON temporaire
      await supabaseClient.storage.from(CHUNK_STORAGE_BUCKET).remove([chunkFilePath]);
      
      return { success: true, message: "Traitement terminé." };
    }

    console.log(`[+] (Batch) Doc ${documentId}, Lot ${batchIndex}: Envoi de ${batchTexts.length} chunks à Cohere...`);
    ...
    // @ts-ignore
    if (!embedResponse.embeddings) throw new Error("Cohere n'a pas retourné d'embeddings.");
    // @ts-ignore
    ...
    const { error: insertError } = await supabaseClient
      ...
      .insert(sections); 
    if (insertError) throw new Error(`Erreur update DB: ${insertError.message}`);

    // 8. RELANCE (Self-Invoke) : Appelle le lot suivant
    console.log(`[+] (Batch) Doc ${documentId}, Lot ${batchIndex}: Terminé. Déclenchement du lot ${batchIndex + 1}...`);
...
serve(async (req) => {
    ...
    try {
        ...
        return new Response(JSON.stringify({ message: "Traitement du lot démarré." }), {
