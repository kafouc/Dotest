...
async function preparePdfForEmbedding(payload: WebhookPayload) {
  ...
  try {
    ...
    const { data: fileData, error: downloadError } =
      await supabaseClient.storage.from('pdfs').download(filePath);
    if (downloadError)
      throw new Error(`Erreur Supabase Storage: ${downloadError.message}`);
    ...
    console.log(
      `[+] (Prepare) Document ${documentId}: ${chunksToEmbed.length} chunks créés.`
    );
    ...
    if (uploadChunkError)
      throw new Error(`Erreur sauvegarde chunks: ${uploadChunkError.message}`);

    // 6. SUCCÈS : Déclenche le premier lot du "worker" d'IA
    console.log(
      `[+] (Prepare) Document ${documentId}: Chunks JSON sauvegardés. Déclenchement du process-batch...`
    );
    ...
    if (invokeError)
      throw new Error(
        `Erreur lors du déclenchement de process-batch: ${invokeError.message}`
      );
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ message: "Préparation de l'analyse démarrée." }),
      {
        status: 200,
